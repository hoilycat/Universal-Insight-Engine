...
*   **Multi-LLM Relay:** 범용 LLM(컨트롤러)**이 전체 파이프라인을 관장하며, 상황에 최적인 오픈소스(Ollama), 클라우드(Groq, Gemini) 모델로 작업을 배분하여 효율과 비용을 최적화.
...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#EEF2FF', 'primaryTextColor': '#1E1B4B', 'primaryBorderColor': '#4F46E5', 'lineColor': '#4F46E5', 'clusterBkg': '#EEF2FF'}}}%%
graph LR
    A["사용자 요청"]:::input --> B

    subgraph Core["🧠 Agentic Core (통합 컨트롤 타워)"]
        direction TB
        B{"LLM 라우터"}:::controller
        J("Ollama / Groq"):::llm_local
        K("Gemini LLM"):::llm_cloud
        F[["🌐 Tavily Search (내장)"]]:::search
        B -->|"단순 분류"| J
        B -->|"복잡한 추론"| K
        J <-.->|"웹 검색 필요 시"| F
        K <-.->|"웹 검색 필요 시"| F
    ...
    subgraph Domain["🔌 Domain Services"]
        direction TB
        C["Domain Selector"]:::selector
        C --> D["🌙 Design Module"]:::module
        C --> E["☕ Coffee Module"]:::module
        C -.->|"Coming Soon"| P["🧳 Packy Module"]:::module_planned
    end
    ...
    subgraph Output["📊 Knowledge & Output"]
        direction TB
        G[("Neo4j<br/>Unified Knowledge Graph")]:::kg_node
    ...
    classDef llm_local fill:#F0F9FF,stroke:#0891B2,color:#1E1B4B,stroke-width:1px
    classDef llm_cloud fill:#EEF2FF,stroke:#4F46E5,color:#312E81,stroke-width:1px
    classDef search fill:#ECFEFF,stroke:#06B6D4,color:#164E63,stroke-width:1.5px,stroke-dasharray:4 4
    ...
    classDef module_planned fill:#F8FAFC,stroke:#94A3B8,color:#64748B,stroke-width:1.5px,stroke-dasharray:4 4
